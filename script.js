...
function animate() {
    ...
    // Acceleration Logic: Increases slowly every frame
    speed += 0.0003; 
    
    stars.position.z += speed; if(stars.position.z > 250) stars.position.z = 0;
    ...
    // Spacing logic adapts to speed
    spawnCounter += speed;
    if(spawnCounter >= (BASE_SPAWN_INTERVAL + (speed * 12))) {
        spawnWall();
        spawnCounter = 0;
    }

    for(let i = walls.length - 1; i >= 0; i--) {
        walls[i].position.z += speed * 4.5;
        
        if(walls[i].position.z > -2 && walls[i].position.z < 6) {
            if(sizeIndex !== walls[i].userData.target) {
                isDead = true;
                document.getElementById('death-screen').classList.remove('hidden');
                document.getElementById('final-stats').innerText = "SYSTEM_CRASH // SCORE: " + score;
        ...
        if(walls[i].position.z > 60) { scene.remove(walls[i]); walls.splice(i, 1); }
    }
